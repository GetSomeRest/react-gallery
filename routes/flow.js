...
var flowFactory = {
  ...
  createFlow: function (rootFolder, resourceId) {

    //console.log('Create Flow:' + resourceId)
    ...
    $.uploadDir = rootFolder + '/' + resourceId;

    try {

      fsExt.mkdirSync($.uploadDir, 0777, true);
    }
    catch (e) {
    ...
    function getChunkFilename(chunkNumber, identifier, useChunks) {
      ...
      return path.resolve(
          $.uploadDir,
          './' + identifier + (useChunks ?'.' + chunkNumber : ''));
    }

    function validateRequest(chunkNumber,
                             chunkSize,
                             totalSize,
                             identifier,
                             filename,
      ...
      if (typeof(fileSize) != 'undefined') {
        ...
        if (numberOfChunks > 1 &&
          ...
          // The chunks in the POST is the last one,
          // and the fil is not the correct size
          return 'invalid_flow_request4';
        }

        if (numberOfChunks == 1 && fileSize != totalSize) {
          // The file is only a single chunk,
          // and the data size does not fit
    ...
    $.get = function (req, useChunks, callback) {
      ...
      if (validateRequest(chunkNumber,
              chunkSize,
              totalSize,
              identifier,
              filename) == 'valid') {
    ...
    $.post = function (req, options, callback) {
      ...
      if (validation == 'valid') {
        ...
        var chunkFilename = getChunkFilename(chunkNumber, identifier, options.useChunks);
        ...
        fs.rename(files[$.fileParameterName].path, chunkFilename, function () {
          ...
          var testChunkExists = function () {
            ...
            fs.exists(getChunkFilename(currentTestChunk, identifier, options.useChunks),
    ...
    $.write = function (fileId, writableStream, options) {
      ...
      var pipeChunkRec = function (number) {

        var chunkFilename = getChunkFilename(number, fileId, options.useChunks);
    ...
    $.clean = function (identifier, options) {
      ...
      var pipeChunkRm = function (number) {
        ...
        var chunkFilename = getChunkFilename(number, identifier, options.useChunks);
